...
class UpdateWarehouseRackRequest extends FormRequest
{
    ...
    {
        $rackId = $this->route('rack')?->id ?? $this->route('id');

        return [
            'zone_id' => ['required', 'exists:warehouse_zones,id'],
            'code'    => [
                'required', 'string', 'max:10',
                Rule::unique('warehouse_racks')->ignore($rackId)->where(fn ($q) => $q->where('zone_id', $this->input('zone_id')))
            ],
    ...
    {
        Log::warning("Validation failed for warehouse rack updating.", [
