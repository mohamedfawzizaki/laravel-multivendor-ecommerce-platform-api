...
class UpdateWarehouseBinRequest extends FormRequest
{
    ...
    {
        $binId = $this->route('bin')?->id ?? $this->route('id');

        return [
            'shelf_id'   => ['required', 'exists:warehouse_shelves,id'],
            'code'       => ['required', 'string', 'max:10',
                             Rule::unique('warehouse_bins')->ignore($binId)->where(fn ($q) => $q->where('shelf_id', $this->input('shelf_id')))],
    ...
    {
        Log::warning("Validation failed for warehouse bin updating.", [
